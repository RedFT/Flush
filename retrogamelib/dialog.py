...
class DialogBox(object):
    ...
    def set_dialog(self, dialog_list):
        ...
        self.shown = True
        self.text_pos = 0
        self.size[0] = self.font.size(self.dialog[0])[0] + 20

        self.size[1] = self.font.get_height() + 8
    ...
    def progress(self):
        ...
        self.size[0] = self.font.size(self.dialog[self.page])[0] + 20
        self.width, self.height = self.size[0], self.size[1]
        self.image = pygame.Surface(self.size).convert_alpha()
        self.update_box()
